...
import PriceHistoryModal from "./ProductDetailModal/PriceHistoryModal";
import SizeModal from "./ProductDetailModal/SizeModal";

import ReactApexChart from "react-apexcharts";

import Carousel from "./../../views/AdminPage/Carousel";
...
class ProductDetail extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.model !== nextProps.model) {
      ...
      nextProps.imageList.forEach((value, index) => {
        imageSlider = [
          ...imageSlider,
          <img
  ...
  sizeModalHandler = bool => {
    this.setState({
      sizeModalOpen: bool
    });
  };

  priceHistoryModalHandler = bool => {
    this.setState({
      priceHistoryModalOpen: bool
    });
  };
  ...
  imageHandler = (imageSrc, clickedIndex) => {
    const { classes } = this.props;

    let mainImage = document.getElementById("main").src;
    let sliderImage = [...this.state.slider];

    let toAdd = [];

    document.getElementById("main").src = imageSrc;

    sliderImage.forEach((value, index) => {
      ...
      if (clickedIndex === index) {
        toAdd = [
          ...toAdd,
          <img
            onClick={() => this.imageHandler(mainImage, index)}
            className={classes.imageListImage}
            id={"slider" + index}
            src={mainImage}
            alt={index}
        ...
        document.getElementById("slider" + index).src = mainImage;
      } else {
        toAdd = [
          ...toAdd,
          <img
            onClick={() => this.imageHandler(value.props.src, index)}
            className={classes.imageListImage}
            id={"slider" + index}
            src={value.props.src}
            alt={index}
  ...
  componentDidMount() {
    window.addEventListener("resize", () => {
      if (this.state.horizontal !== window.innerWidth > 959) {
        this.setState({ horizontal: window.innerWidth > 959 });
      }
    });
  }
  ...
  componentWillMount() {
    ...
    this.props.imageList.forEach((value, index) => {
      imageSlider = [
        ...imageSlider,
        <img
          onClick={() => this.imageHandler(value, index)}
          className={classes.imageListImage}
          id={"slider" + index}
          src={value}
          alt={index}
  ...
  sizeHandler = size => {
    ...
    this.props.sizeChanger(size)
  };

  imageWrapperHandler = () => {
    const { classes } = this.props;

    if (this.state.priceHistoryModalOpen) {
      return (
        <ClickAwayListener
          onClickAway={() => this.priceHistoryModalHandler(false)}
        >
          <PriceHistoryModal sid={this.props.sid} size={this.state.sizeModal} />
        </ClickAwayListener>
      );
    }

    if (this.state.sizeModalOpen) {
      return (
        <ClickAwayListener onClickAway={() => this.sizeModalHandler(false)}>
          <SizeModal
            sid={this.props.sid}
      ...
      >
        ...
        >
          ...
          <div className={classes.imageContainer}>
            <div
              className={
                this.state.sizeModalOpen || this.state.priceHistoryModalOpen
                  ? classes.withImageActive
                  : classes.withImage
              }
            >
              <div
                className={
                  this.state.sizeModalOpen || this.state.priceHistoryModalOpen
                    ? classes.imageWrapperOther
                    : classes.imageWrapper
                }
              >
                {this.imageWrapperHandler()}
                {/* <img
                  src={this.props.image}
                  alt={this.props.model}
                  className={classes.image}
            ...
            <Carousel horizontal={this.state.horizontal} detail={true}>
              {this.state.slider}
            </Carousel>
          </div>
        </div>
        ...
        >
          <Button
            ...
            classes={{
              ...
              label: classes.buttonLable
            }}
            className={classes.lowestButton}
            onClick={() => this.sizeModalHandler(true)}
          >
            {this.state.unreleased
              ? "아직 출시되지 않은 신발입니다."
              : this.props.stockCount
              ? "사이즈 선택"
              : "품절"}
            <div>{this.state.sizeModal ? this.state.sizeModal : "All"}</div>
          </Button>
          <Button
            disabled={true}
            classes={{
          ...
          >
            {this.state.unreleased
              ? "아직 출시되지 않은 신발입니다."
              : this.props.stockCount
              ? "최저가 ₩ " + this.props.lowest.toLocaleString()
              : "품절"}
          </Button>
          ...
          <Button
            ...
            className={classes.lowestButton}
            onClick={() => this.priceHistoryModalHandler(true)}
          >
            {this.state.unreleased
              ? "아직 출시되지 않은 신발입니다."
              : this.props.stockCount
          ...
          {this.state.sizeOpen ? (
            <ClickAwayListener onClickAway={this.sizePanelClose}>
              <div className={classes.sizeContainer}>{this.state.size}</div>
            </ClickAwayListener>
          ) : (
            // <ClickAwayListener onClickAway={this.sizePanelClose}>
            <Button
        ...
        <div className={classes.sectionContainer}>
          <RelatedSection
            headerName="연관 상품"
            helper={korean + " 와 관련된 상품입니다"}
            shoes={this.props.related}
          />
        </div>
        {/* <div className={classes.description}>
          <div className={classes.modelDetail}>{this.props.model}</div>
          신발에 대한 설명 부분이 들어가는 곳 입니다.
        </div> */}
        <div className={classes.description}>
        ...
        <div className={classes.recommendButtonContainer}>
          <div className={classes.recommendContainerLeft}>
            <div className={classes.recommendNumber}>2000</div>
            <Button className={classes.recommendButtonUp}>추천</Button>
          </div>
          <div>
            <div className={classes.recommendNumber}>50</div>
            <Button className={classes.recommendButtonDown}>비추천</Button>
          </div>
        </div>
...
export default withStyles(propageStyle)(ProductDetail);
